...
export interface FormatStackTraceOptions {
  ...
  syncOnly?: boolean;
  /** How to display frames with no URL. 'anonymous' (default) shows "<anonymous>", 'vm' shows "VM{scriptId}". */
  locationFallback?: 'anonymous' | 'vm';
}
...
export function formatStackTrace(
  ...
  const {
    maxFrames = Infinity,
    includeAsyncFragments = true,
    syncOnly = false,
    locationFallback = 'anonymous',
  } = options;
  const lines: string[] = [];
  ...
  while (current && frameCount < maxFrames) {
    for (const frame of current.callFrames) {
      ...
      const location =
        frame.url ||
        (locationFallback === 'vm' ? `VM${frame.scriptId}` : '<anonymous>');
      lines.push(
        `    at ${functionName} (${location}:${frame.lineNumber + 1}:${frame.columnNumber + 1})`,
      );
      frameCount += 1;
    }
  ...
  const totalFrames = syncOnly
    ? stack.callFrames.length
    : countStackFrames(stack);
